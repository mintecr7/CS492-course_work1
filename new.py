...
actual_picture = avgFace1 + A1[:,100].reshape(-1,1)
error = []
for j in range(100):
    rec1 = (w[300, 0]*P1[:, 0]).reshape(-1,1)
    for i in range(1, j):
        rec1 += (P1[:, i]*w[104, i]).reshape(-1,1)
    difference = rec1.reshape(-1,1) - A1[:, 100].reshape(-1,1)
    error.append(np.linalg.norm(difference)/np.linalg.norm(A1[:, 100]))
...
print("Face recognition accuracy of batch 1 incremental PCA: ", accuracy)
